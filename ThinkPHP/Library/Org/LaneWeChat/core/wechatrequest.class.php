                ...
                switch ($request['event']) {
                    ...
                    case 'subscribe':
                        ...
                        if(isset($request['eventkey']) && isset($request['ticket'])){
                            $id=str_replace('qrscene_', '', $request['eventkey']);
                            $aOb=M('Active');
                            $re=$aOb->field('cover,address,title')->where('id='.$id)->find();
                          //  var_dump($re);die;
                            $tuwenList[] =array('title'=>urlencode($re['title']), 'description'=>urlencode($re['address']) , 'pic_url'=>urlencode(C('YUMING').'/Public/Uploads/thumb/'.$re['cover']), 'url'=>urlencode(C('YUMING').U('Home/Active/advanceDetail',array('id'=>$id)))) ;
                            $itemList = array();
                          ...
                          // $data = self::link("http://www.baidu.com");
