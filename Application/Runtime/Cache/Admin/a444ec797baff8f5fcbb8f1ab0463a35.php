		...
		<section>
			<table class="table">
				...
				<?php if(is_array($arr)): foreach($arr as $key=>$v): ?><tr>
					...
					<td><?php echo ($v["admin_action"]); ?></td>
					<td><?php echo ($v["log_ip"]); ?></td>
					<td><?php echo (date("Y-m-d H:i:s",$v["log_time"])); ?></td>
					<td>
					<a href="#" onclick='del(<?php echo ($v["log_id"]); ?>)' class="inner_btn2">删除</a>
					</td>
...
function del(log_id){
	$.ajax({
        type:'post',
        url:"<?php echo U('Admin/Admin/logDel');?>?log_id="+log_id,
        dataType:'json',
        success:function(data){ 
          	if (data=='success') {
          		alert('删除成功！');
          		window.location.href="<?php echo U('Admin/Admin/adminLog');?>";
          	}else{
          		alert('删除失败！');                                          
        	}                          
        }
