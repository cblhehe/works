    ...
    <section>
      <table class="table">
        ...
        <?php if(is_array($re)): foreach($re as $key=>$v): ?><tr>
          ...
          <td>
          <a href="<?php echo U('Admin/User/levelModify');?>?level_id=<?php echo ($v["level_id"]); ?>" class="inner_btn">修改</a>
          <a href="#" onclick='del(<?php echo ($v["level_id"]); ?>)' class="inner_btn2">删除</a>
          </td>
...
function del(level_id){
  $.ajax({
        type:'post',
        url:"<?php echo U('Admin/User/levelDel');?>?level_id="+level_id,
        dataType:'json',
