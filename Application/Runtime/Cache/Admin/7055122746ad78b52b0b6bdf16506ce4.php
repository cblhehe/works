...
<section class="rt_wrap content mCustomScrollbar">
 ...
 <section>
  <ul class="admin_tab">
    <li><a  class="active">待接单</a></li>
    <li><a  href="<?php echo U('Admin/Order/waitCheck');?>">待入住</a></li>
    ...
    <section>
      <table class="table">
        ...
        <?php if(is_array($arr)): foreach($arr as $key=>$v): ?><tr>
          ...
          <a href="#" class="inner_btn"  onclick="receive(<?php echo ($v["order_id"]); ?>)">接单</a>
...
function receive(order_id){
  var url="<?php echo U('Admin/Order/receive');?>?order_id="+order_id+'&order_status=2';
    $.ajax({  
        type: "post",  
        url: url,  
        dataType: "json",  
        success: function(data){  
          if (data=='success') {
            alert('订单状态更改成功！');
            window.location.href="<?php echo U('Admin/Order/stayWaitReceive');?>";
          }else{
            alert('订单状态更改失败！');
          }
        }  
