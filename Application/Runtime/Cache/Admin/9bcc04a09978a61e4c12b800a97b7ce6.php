	...
	<div class="admin_tab_cont" style="display:block;" id="tab1">
		<ul class="ulColumn2">
			<li>
				...
				<input type="text" class="textbox textbox_225" name="username" id="username" value="<?php echo ($re["username"]); ?>" />
				<input type="hidden" name="oldName" value="<?php echo ($re["username"]); ?>" id="oldName">
			...
			<li>
				...
				<input type="password" class="textbox textbox_225" name="password" id="password" value="<?php echo ($re["password"]); ?>"/>
				<input type="hidden" name="oldPwd" value="<?php echo ($re["password"]); ?>">
				<span class="errorTips" id="w2">密码不能为空！</span>
			</li>
			<li>
		        <span class="item_name" style="width:120px;">管理员角色：</span>
		        <label class="single_selection">
		        <input type="radio" name="role" value="0"  <?php if($re["role"] == 0 ): ?>checked='checked'<?php endif; ?>/>普通管理员</label>
		        <label class="single_selection">
		        <input type="radio" name="role" value="2" <?php if($re["role"] == 2 ): ?>checked='checked'<?php endif; ?>/>中级管理员</label>
		        <label class="single_selection">
		        <input type="radio" name="role" value="1" <?php if($re["role"] == 1 ): ?>checked='checked'<?php endif; ?>/>超级管理员</label>
	       	</li>
...
$('#submit').click(function(){
	...
	console.log(params);
    var url = "<?php echo U('Admin/Admin/reSave');?>?admin_id=<?php echo ($re["admin_id"]); ?>";  
    $.ajax({  
        type: "post",  
        url: url,  
        dataType: "json",  
