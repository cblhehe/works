...
<section class="rt_wrap content mCustomScrollbar">
 ...
 <section>
	...
	<form action="<?php echo U('Admin/Goods/roomSave');?>?room_id=<?php echo ($re["room_id"]); ?>"  method="post" enctype="multipart/form-data">
		<ul class="ulColumn2">
			<li>
				<span class="item_name" style="width:120px;">住房名称：</span>
				<input type="text" class="textbox textbox_295 content" name="room_name" id="room_name" value="<?php echo ($re["room_name"]); ?>" />
			...
			<li>
		        ...
		        <select class="select" name="room_cate" id="room_cate">
		         	...
		         	<option value="1" <?php if($re["room_cate"] == 1 ): ?>selected='selected'<?php endif; ?>>名宿</option>
		         	<option value="2" <?php if($re["room_cate"] == 2 ): ?>selected='selected'<?php endif; ?>>酒店</option>
		        </select>
	       ...
	       <li>
		        ...
		        <select class="select" name="room_type" id="room_type">
		         	...
		         	<option value="1" <?php if($re["room_cate"] == 1 ): ?>selected='selected'<?php endif; ?>>单租</option>
		         	<option value="2" <?php if($re["room_cate"] == 2 ): ?>selected='selected'<?php endif; ?>>整租</option>
		        </select>
		        <span class="errorTips" id="t3">请选择房间类型！</span>
	       </li>
			<li>
				<span class="item_name" style="width:120px;">可住人数：</span>
				<input type="text" class="textbox textbox_70 content" name="population" id="population"  value="<?php echo ($re["population"]); ?>"/>&nbsp;&nbsp;人
				<span class="errorTips" id="t4">请填写可住人数！</span>
			</li>
			<li>
				<span class="item_name" style="width:120px;">床位：</span>
				<input type="text" class="textbox textbox_70 content" name="bed_num" id="bed_num"  value="<?php echo ($re["bed_num"]); ?>"/>&nbsp;&nbsp;张
			...
			<li>
				...
				<input type="text" class="textbox textbox_70 content" name="rental" id="rental"  value="<?php echo ($re["rental"]); ?>"/>&nbsp;&nbsp;元/天
			...
			<li>
		        ...
		        <label class="single_selection"><input type="radio" name="toilet" value="1" <?php if($re["toilet"] == 1 ): ?>checked='checked'<?php endif; ?> />是</label>
		        <label class="single_selection"><input type="radio" name="toilet" value="0" <?php if($re["toilet"] == 0 ): ?>checked='checked'<?php endif; ?>/>否</label>
	       </li>
	       <li>
		        <span class="item_name" style="width:120px;">是否可加床位：</span>
		        <label class="single_selection"><input type="radio" name="add_bed" value="1" <?php if($re["add_bed"] == 1 ): ?>checked='checked'<?php endif; ?>/>是</label>
		        <label class="single_selection"><input type="radio" name="add_bed" value="0" <?php if($re["add_bed"] == 0 ): ?>checked='checked'<?php endif; ?>/>否</label>
	       </li>
	       <li>
		        <span class="item_name" style="width:130px;">是否接待境外人士：</span>
		        <label class="single_selection"><input type="radio" name="accept_foreigner" value="1" <?php if($re["accept_foreigner"] == 1 ): ?>checked='checked'<?php endif; ?>/>是</label>
		        <label class="single_selection"><input type="radio" name="accept_foreigner" value="0" <?php if($re["accept_foreigner"] == 0 ): ?>checked='checked'<?php endif; ?>/>否</label>
	       </li>
	       <li>
		        ...
		        <select class="select" name="least_day" id="least_day">
		         	...
		         	<?php if(is_array($day)): foreach($day as $key=>$v): ?><option value="<?php echo ($v["day"]); ?>" <?php if($re["least_day"] == $v["day"] ): ?>selected='selected'<?php endif; ?>><?php echo ($v["day"]); ?>天</option><?php endforeach; endif; ?>
		        </select>
		        <!-- <span class="errorTips" id="t9">选择最少入住天数!</span> -->
	       </li>
			<li>
				<span class="item_name" style="width:120px;">住房地址：</span>
				<input type="text" class="textbox textbox_295 content" name="address" id="address" value="<?php echo ($re["address"]); ?>"/>
				<span class="errorTips" id="t10">住房地址不能为空！</span>
			</li>
			<li>
				<span class="item_name" style="width:120px;">住房描述：</span>
				<textarea  class="textarea" style="width:500px;height:100px;" name="description" id="description"><?php echo ($re["description"]); ?></textarea>
				<span class="errorTips" id="t11">请填写住房描述！</span>
			</li>
			<li>
				<span class="item_name" style="width:120px;">交通描述：</span>
				<textarea  class="textarea" style="width:500px;height:100px;" name="traffic" id="traffic"><?php echo ($re["traffic"]); ?></textarea>
				<span class="errorTips" id="t12">请填写交通描述！</span>
			</li>
			
	       <li>
				<span class="item_name" style="width:120px;">预订条款：</span>
				<textarea  class="textarea" style="width:500px;height:100px;" name="reservation_terms" id="reservation_terms"><?php echo ($re["reservation_terms"]); ?></textarea>
			</li>
			<li>
				...
				<div style="padding:30px 0 0 120px;" id="img_wrap">
					<?php if(is_array($images)): foreach($images as $key=>$v): ?><img src='/sy/Public/Uploads/<?php echo ($v["room_image"]); ?>'  class='oldImg' height='82' alt='' style='margin:-22px 0px 18px 10px;' />
					<a style='margin: 5px;' href="#" onclick="delImg(this,<?php echo ($v["image_id"]); ?>,'<?php echo ($v["room_image"]); ?>',<?php echo ($re["room_id"]); ?>)">删除</a><?php endforeach; endif; ?>
				</div>
			</li>
...
function delImg(obj,image_id,room_image,room_id){
	var url="<?php echo U('Admin/Goods/imageDelete');?>?image_id="+image_id+'&room_id='+room_id;
	$.ajax({  
	    type: "post",  
	    url: url,  
	    dataType: "json",  
