    ...
    <section>
      <table class="table">
        ...
        <?php if(is_array($arr)): foreach($arr as $key=>$v): ?><tr>
          ...
          <td><?php echo ($v["commenter"]); ?></td>
          <td><?php echo ($v["responser"]); ?></td>
          <td><?php echo ($v["comment_content"]); ?></td>
          <td><?php echo ($v["response_content"]); ?></td>
          <td><?php echo (date("Y-m-d H:i:s",$v["comment_time"])); ?></td>
          <td>
          <?php if(!empty($$v["response_time"])): echo (date("Y-m-d H:i:s",$v["response_time"])); endif; ?>
          </td>
          <td>
          <?php if(empty($v["response_content"])): ?><a href="#" class="inner_btn"  onclick="response(<?php echo ($v["comment_id"]); ?>)">回复</a>
          <?php else: ?>
          <a href="#" class="inner_btn"  style="background-color:#fff;padding-right:43px;"></a><?php endif; ?>
         <a href="<?php echo U('Admin/Order/commentDelete');?>?comment_id=<?php echo ($v["comment_id"]); ?>" class="inner_btn2">删除</a>
          </td>
...
function response(comment_id){
  ...
  if (content==='') {
    ...
    var url="<?php echo U('Admin/Order/commentResponse');?>?comment_id="+comment_id;
    $.ajax({  
        type: "post",  
        url: url,  
        ...
        success: function(data){  
          if (data=='success') {
            alert('回复成功！');
            window.location.href="<?php echo U('Admin/Order/commentList');?>";
          }else{
            alert('回复失败！');
          }
        }  
